    ...
    if (user) {
        ...
        database.ref('users/' + userId).once('value')
            .then((snapshot) => {
                ...
                document.getElementById('surnameDisplay').innerText = userData.surname;
...
function loadTodaysSchedule(userId) {
    ...
    database.ref('users/' + userId + '/tasks').orderByChild('deadline').equalTo(today).on('value', snapshot => {
        const tasks = snapshot.val();
        if (tasks) {
            Object.keys(tasks).forEach(key => {
                const task = tasks[key];
